...
void AntGUIDialog::AddImageText(int id, LPCWSTR text, int x, int y, int h, int w)
{
...
void AntGUIDialog::AddButton(int id, LPCWSTR text, int x, int y, int h, int w)
{
...
void AntGUIDialog::AddCheckBox(int id, LPCWSTR text, int x, int y, int h, int w)
{
...
void AntGUIDialog::AddTextBox(int id, LPCWSTR text, int x, int y, int h, int w)
{
...
void AntGUIDialog::AddTextArea(int id, LPCWSTR text, int x, int y, int h, int w)
{
...
void AntGUIDialog::AddListBox(int id, LPCWSTR text, int x, int y, int h, int w)
{
...
bool AntGUIDialog::Event( UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	if (!_visible || !_enabled) { return false; }
	switch( uMsg )
    {
		case WM_KEYDOWN :
			if (_focus!=NULL) { _focus->Event(uMsg, wParam, lParam); return false; }
		break;
		case WM_LBUTTONUP:
			if (_focus!=NULL) { _focus->Event(uMsg, wParam, lParam); return false; }
		break;
		...
		case WM_MOUSEMOVE:
				...
				if (uMsg == WM_LBUTTONDOWN) { 
					if (intersect!=NULL) { _focus=intersect; _focus->Event(uMsg, wParam, lParam); }
					else { _focus=NULL; }
					return false;
				}
				...
				if (uMsg == WM_MOUSEMOVE) {
					...
					else if (intersect!=NULL && intersect==_focus)	{ _focus->Event(uMsg, wParam, lParam); }
				}
		break;
	}
...
void AntGUIDialog::DrawText(UINT fontId, LPCWSTR string, AntFontColorARGB* fontColor, RECT* dst)
{
...
void AntGUIImageText::SetText(LPCWSTR text)
{
	StringCchCopy(_text,MAX_PATH,text);
}
...
bool AntGUIButton::Event( UINT uMsg, WPARAM wParam, LPARAM lParam )
{ 
	switch (uMsg)
	{
	case WM_KEYDOWN :
		if (wParam==VK_RETURN) { _pressed=true; }
	break;
	case WM_LBUTTONDOWN :
		_pressed = true;
		return true;
...
void AntGUIComboBox::AddOption(int id, LPCWSTR text, void** userData)
{
	AntGUIComboBoxOption option;
	StringCchCopy(option._text,MAX_PATH,text);
	option._id=id;
...
bool AntGUIComboBox::Event( UINT uMsg, WPARAM wParam, LPARAM lParam )
{ 
	switch (uMsg)
	{
	case WM_LBUTTONDOWN :
	case WM_MOUSEMOVE:
		if (_opened)
		{
			...
			{
				_hover=NULL;
				if (uMsg==WM_LBUTTONDOWN) { _opened=false; UpdateBoundingBox();}
			} else
			{
				if (uMsg==WM_LBUTTONDOWN)
				{
		...
		{
			if (uMsg==WM_LBUTTONDOWN) { _opened=true; UpdateBoundingBox();}
		}
	break;
	}
	return false;
...
bool AntGUICheckBox::Event( UINT uMsg, WPARAM wParam, LPARAM lParam )
{ 
	switch (uMsg)
	{
	case WM_LBUTTONDOWN :
...
bool AntGUITextBox::Event( UINT uMsg, WPARAM wParam, LPARAM lParam )
{ 
	switch (uMsg)
	{
	case WM_KEYDOWN :
		switch(wParam)
		{
			...
			break;
			case VK_BACK:
				_text[_pos]=(WCHAR)NULL;
				SetCaret(_pos-1);
			break;
			default :
				_text[_pos]=(WCHAR)wParam;
				SetCaret(_pos+1);
			break;
		}
	break;
...
bool AntGUISlider::Event( UINT uMsg, WPARAM wParam, LPARAM lParam ) { return false; }
