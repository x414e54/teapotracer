                     ...
                     int       nCmdShow)
{
	...
	Settings.Read();
	rotation=0.0f;
	if(!AntWrapInit(&Renderer, Settings, hInstance, L"TeapotRacer")) { MessageBox(0, L"DisplayInitFailed(CannotContinue)", 0, 0); return -1;}
...
bool Event(UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (Game.GetState()==ANTGAME_STATE_MENU)						// Start Menu state, we just need to allow quit and send everythign else off to the GUI
	{
		...
		StartDlg.Event(msg, wParam, lParam);
	...
	{																// we must then send all other events to the GUI's
		...
		MenuDlg.Event(msg, wParam, lParam);
		Controls->Event(msg, wParam, lParam);
